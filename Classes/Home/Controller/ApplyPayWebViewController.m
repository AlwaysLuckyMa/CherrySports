//
//  ApplyPayWebViewController.m
//  CherrySports
//
//  Created by dkb on 16/12/13.
...
@interface ApplyPayWebViewController ()<UIWebViewDelegate, NJKWebViewProgressDelegate, WXApiDelegate, WXApiManagerDelegate, aliPayDelegate>{
    UIWebView *_webView;
...
@implementation ApplyPayWebViewController
...
//    NSString *httpStr = @"http://192.168.1.132:8081/cherryitf/v1/enter/jumpPay?orderId=19b5202f-e096-41a6-a425-ef3ad77ba306&totalPrice=20.00&payChannel=0";
    ...
    NSURLRequest *request = [NSURLRequest requestWithURL:[NSURL URLWithString:[AppTools httpsWithStrUser:_tEnterUrl]]];
...
- (void)setNavigationController
{
    ...
    if ([self.tName isEqualToString:@"自己报名"]) {
        // 导航栏标题（用图片）
        UILabel *titleLabel = [AppTools createLabelText:@"自己报名" Color:[UIColor whiteColor] Font:16 TextAlignment:NSTextAlignmentCenter];
        titleLabel.font = [UIFont systemFontOfSize:16];
        titleLabel.frame = CGRectMake(0, 0, 100, 30);
        titleLabel.userInteractionEnabled = YES;
        self.navigationItem.titleView = titleLabel;
    }
    else if ([self.tName isEqualToString:@"多人报名"])
    {
        // 导航栏标题（用图片）
        UILabel *titleLabel = [AppTools createLabelText:@"多人报名" Color:[UIColor whiteColor] Font:16 TextAlignment:NSTextAlignmentCenter];
        titleLabel.font = [UIFont systemFontOfSize:16];
        titleLabel.frame = CGRectMake(0, 0, 100, 30);
        titleLabel.userInteractionEnabled = YES;
        self.navigationItem.titleView = titleLabel;
    ...
    {
        // 导航栏标题（用图片）
        UILabel *titleLabel = [AppTools createLabelText:@"报名信息" Color:[UIColor whiteColor] Font:16 TextAlignment:NSTextAlignmentCenter];
        titleLabel.font = [UIFont systemFontOfSize:16];
        titleLabel.frame = CGRectMake(0, 0, 100, 30);
        titleLabel.userInteractionEnabled = YES;
        self.navigationItem.titleView = titleLabel;
