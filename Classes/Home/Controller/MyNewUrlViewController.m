...
@interface MyNewUrlViewController ()<UIWebViewDelegate, NJKWebViewProgressDelegate, WXApiDelegate, WXApiManagerDelegate, aliPayDelegate>{
    UIWebView *_webView;
...
@implementation MyNewUrlViewController
...
-(void) settingWebView {
    ...
    if ([[NSUserDefaults standardUserDefaults] objectForKey:@"locationJD"] != nil && [[NSUserDefaults standardUserDefaults] objectForKey:@"locationWD"] != nil) {
         string = [NSString stringWithFormat:@"%@&tAppUserId=%@&appcode=iOS&jumpPosition=2&longitude=%@&latitude=%@&sJis=%zd", _urlNew, USERID, LOCATIONJD, LOCATIONWD, value];
    }else{
        string = [NSString stringWithFormat:@"%@&tAppUserId=%@&appcode=iOS&jumpPosition=2&sJis=%zd", _urlNew, USERID, value];
    }
...
//    
//    NSDictionary *tIdDic = @{@"dicKey":@"tGamesId", @"data":@"123123"};
//    NSDictionary *userIdDic =@{@"dicKey":@"tAppUserId", @"data":userId};
//    
//    NSArray *postArr = @[tIdDic, userIdDic];
//    
//    NSString *url = [NSString stringWithFormat:@"%@/user/insertVerifyCode", SERVER_URL];
//    
