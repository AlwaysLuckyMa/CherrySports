 ...
 */
- (void)setShowHighQualityImageWithURL:(NSURL *)url placeholderImage:(UIImage *)placeholder //1996
{
    if (!url) {
...
//    hud.labelText = @"图片加载中..." ;
//    hud.labelFont = [UIFont italicSystemFontOfSize:14.f];
...
//    self.progressView.mode = XLProgressViewProgressMode;
    ...
    [weakSelf.photoImageView sd_setImageWithURL:url
        ...
        if (expectedSize>0)
        {
            [self.zoomingScrollViewdelegate SettingDelegateNum:self
                                                       loadNum:expectedSize
                                                     loadAlNum:1.0 * receivedSize
                                                        allNum:(CGFloat)receivedSize / expectedSize];
...
//            [hud hide:YES];
        ...
        NSLog(@"图片加载完成--load_over");
        [[NSNotificationCenter defaultCenter] postNotificationName:@"load_over" object:nil];
    }];
}
